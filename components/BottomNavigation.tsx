...
export default function BottomNavigation() {
  ...
  return (
    <div className="h-[60px] flex items-center border-t border-zinc-200 bg-white">
      <div className="max-w-md mx-auto flex items-center justify-around w-full px-4">
        {navigationItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={`w-6 h-6 ${isActive ? "text-brand-600" : "text-zinc-400"}`} />
              <span
                className={`text-xs font-medium transition-all duration-200 ${
                  isActive ? "text-brand-700 font-semibold" : "text-zinc-400"
                }`}
              >
                {item.name}
              </span>
